...
export function validateDebtDraft(fields) {
    const errors = [];
    ...
    if (!overdue_period)
        errors.push("Overdue period cannot be empty.");
    const relationship = fields.relationship?.trim();
    if (!relationship)
        errors.push("Relationship cannot be empty.");
    const priorReminderText = fields.prior_reminder?.trim().toLowerCase();
    const prior_reminder = priorReminderText === "yes" || priorReminderText === "y"
        ? true
        : priorReminderText === "no" || priorReminderText === "n"
            ? false
            : undefined;
    if (prior_reminder === undefined)
        errors.push("Previous reminder must be Yes or No.");
    if (errors.length > 0)
