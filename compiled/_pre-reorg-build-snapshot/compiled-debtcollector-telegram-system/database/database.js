...
export function saveDebt(input) {
    const database = getDb();
    const created_at = new Date().toISOString();
    const stmt = database.prepare(`INSERT INTO debts (name, amount, reason, overdue_period, relationship, prior_reminder, context, tone, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`);
    const result = stmt.run(input.name, input.amount, input.reason, input.overdue_period, input.relationship, input.prior_reminder ? 1 : 0, input.context ?? null, input.tone ?? null, created_at);
    return {
