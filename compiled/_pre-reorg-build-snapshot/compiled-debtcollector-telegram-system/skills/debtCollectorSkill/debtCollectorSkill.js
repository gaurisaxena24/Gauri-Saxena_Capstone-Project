 ...
 */
import { startSession, getSession, clearSession, } from "../debtFormStore/debtFormStore.js";
...
const SKIPPABLE_FIELDS = new Set(["additional_context", "tone"]);
...
function isAwaitingReminderCount(session) {
    return (session.fields.previously_reminded === true &&
        session.fields.previous_reminder_count === undefined);
}
function toDebtContext(fields) {
    return {
...
function buildResult(session, options = {}) {
 ...
 * stop.
 */
export function invokeDebtCollectorSkill(chatId, rawText) {
