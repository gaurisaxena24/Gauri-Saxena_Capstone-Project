 ...
 */
import { tgSendMessage, tgEditMessageText, tgAnswerCallbackQuery, } from "../../telegram/rawApi.js";
import { createDebtDraft, getDebtDraft, setDebtDraftLiveMessage, setDebtDraftStatus, updateDebtDraftFields, findAwaitingEditDraftByChat, } from "../../debtDraft/debtDraftStore.js";
import { formatDebtPreview, buildDebtPreviewKeyboard, formatEditableDraftText, formatSavedSuffix, parseEditedDraftText, } from "../../debtDraft/debtDraftFormat.js";
import { validateDebtDraft } from "../../debtDraft/debtDraftValidation.js";
import { saveDebt } from "../../database/database.js";
function debtContextToDraftFields(ctx) {
    return {
...
export async function startDebtDraftReview(chatId, debtContext) {
    const draft = createDebtDraft(chatId, debtContextToDraftFields(debtContext));
    const sent = await tgSendMessage(chatId, formatDebtPreview(draft.fields), buildDebtPreviewKeyboard(draft.draftId));
 ...
 */
export async function handleDebtDraftCallback(cb) {
    if (!cb.data || !cb.message)
        return false;
    const [action, draftId] = cb.data.split(":");
    if (action !== "save_debt" && action !== "edit_debt")
        return false;
    ...
    if (action === "save_debt") {
        ...
        await tgEditMessageText(cb.message.chat.id, cb.message.message_id, `${cb.message.text ?? formatDebtPreview(draft.fields)}${formatSavedSuffix(saved.id)}`);
 ...
 */
export async function handleDebtDraftEditReply(message) {
    ...
    if (!validation.valid) {
        await tgSendMessage(message.chat.id, `Please correct and resend:\n- ${validation.errors.join("\n- ")}`);
        return true;
    }
    updateDebtDraftFields(draft.draftId, updatedFields);
    setDebtDraftStatus(draft.draftId, "pending_review");
    const sent = await tgSendMessage(message.chat.id, formatDebtPreview(updatedFields), buildDebtPreviewKeyboard(draft.draftId));
    setDebtDraftLiveMessage(draft.draftId, sent.message_id);
    return true;
}
async function answerSafely(callbackQueryId, text) {
    try {
