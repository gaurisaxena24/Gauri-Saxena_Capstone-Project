...
export const createDebtReminderDraftInputSchema = z.object({
    ...
    recipient_chat_id: z
        ...
        .optional()
        .describe("Telegram chat ID of the final recipient. If omitted, the approved message is sent " +
        "back to the reviewer's own chat in TEST MODE instead of a real recipient."),
});
export async function createDebtReminderDraft(input) {
    const parsed = createDebtReminderDraftInputSchema.parse(input);
    const reviewerChatId = process.env.TELEGRAM_REVIEWER_CHAT_ID;
    if (!reviewerChatId) {
        return {
            success: false,
            message: "TELEGRAM_REVIEWER_CHAT_ID is not configured. Set it in your .env file.",
        };
    }
    ...
    try {
        const sent = await tgSendMessage(reviewerChatId, formatReviewMessage({
    ...
    return {
        success: true,
        message: "Draft sent to Telegram for review. Nothing will be sent to the recipient until you tap Approve.",
        draftId: draft.draftId,
        draftText,
    };
}
