...
export function buildDebtPreviewKeyboard(draftId: string): InlineKeyboardMarkup {
  return {
    inline_keyboard: [
      [
...
export function formatEditableDraftText(fields: DebtDraftFields): string {
  return (
    `Edit any line below and send it back as a single message (keep the "Label:" prefixes). ` +
 ...
 */
export function parseEditedDraftText(text: string, base: DebtDraftFields): DebtDraftFields {
  const updated: DebtDraftFields = { ...base };
